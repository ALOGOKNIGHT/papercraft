...
export default function PaperPreview({ meta, layout, questions, hideControls, isEmbedded }: Props) {
      ...
      sections: [{
        ...
        children: [
          ...
          ...(meta.headerType === 'custom' ? [
            new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Custom Header (See PDF for full formatting)", bold: true, size: 24, color: "888888" })] }),
            new Paragraph({ text: "" })
          ] : [
            new Paragraph({ children: [new TextRun({ text: meta.examDate || '23/06/2025', bold: true, size: 24 })] }),
            new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "[ Logo / Institute Name Image Placeholder ]", bold: true, size: 24, color: "999999" })] }),
            new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Corporate Office : Chaturavton ka bera, Ponjala, Mata ka than road, Jodhpur,", bold: true, size: 20 })] }),
            new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Ph. 9829764378,8619186133", italics: true, size: 20 })] }),
            new Paragraph({ text: "" }),

            // 2. Exam Meta Data (Time, Marks, etc)
            new Paragraph({
              children: [
            ...
            ...(meta.topicsCovered ? meta.topicsCovered.split('\n').filter(Boolean).map((line) => new Paragraph({ children: [new TextRun({ text: line, size: 24 })] })) : [
                ...
                rows: [
                  ...qs.map(q => {
                    ...
                    if (q.type === 'MCQ' && q.options && q.options.length > 0) {
                      cellParagraphs.push(new Paragraph({ text: "" })); // spacing
                      if (layout.mcqLayout === '4-col') {
                        ...
                        q.options.forEach((opt, i) => {
                          cellParagraphs.push(new Paragraph({ text: `(${String.fromCharCode(97 + i)}) ${opt}` }));
                        });
                      } else {
                        // 2-col (default)
  ...
  // Trigger print with slight delay to allow React to apply the CSS changes
  ...
  return (
    <div
      ...
      style={{
      background: isEmbedded ? 'transparent' : '#d4cfc4',
      ...
      <style dangerouslySetInnerHTML={{
        ...
        @media print {
          ...
          /* Dynamically tell the browser to rotate the paper */
          @page {
          ...
          /* Strip the editor look during print */
          .paper-sheet {
      ...
      {/* --- ACTION BUTTONS --- */}
      {!hideControls && (
        <div className="no-print" style={{ textAlign: 'center', marginBottom: '24px', display: 'flex', justifyContent: 'center', gap: '15px' }}>
          <button onClick={() => handlePrint('single')} style={{ padding: '12px 24px', background: '#4a4a4a', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '700' }}>
            🖨 Print 1-Per-Sheet (Portrait)
          </button>
          <button onClick={() => handlePrint('dual')} style={{ padding: '12px 24px', background: 'linear-gradient(135deg,#e2b96a,#c9953a)', color: '#1a1a2e', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '700' }}>
            📖 Print 2-Per-Sheet (Landscape)
          </button>
          <button onClick={exportToWord} style={{ padding: '12px 24px', background: '#2b579a', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '700' }}>
            📥 Download Word
          </button>
        </div>
      )}
      ...
      {/* --- THE EXAM PAPER --- */}
      {/* If mode is 'dual', we apply CSS columns. 
          The browser will automatically flow content into two halves separated by a line. 
      */}
      <div className="paper-sheet" style={{
        ...
        {layout.watermarkEnabled && meta.logo && (
          <img
            ...
            style={{
              position: 'absolute',
              top: '148.5mm', // Center of first A4 page in preview
              left: '50%',
        ...
        {/* 1. DPS PROFESSIONAL HEADER (Break inside avoid ensures it stays together) */}
        <div style={{ breakInside: 'avoid', marginBottom: '20px' }}>
          {meta.headerType === 'custom' ? (
            <div
              className="custom-header-preview"
              dangerouslySetInnerHTML={{ __html: meta.customHeaderHTML || '<div style="text-align: center; color: #999;">Empty Custom Header</div>' }}
            />
          ) : (
            <>
              <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '15px', zIndex: 1 }}>
                ...
                <div style={{ fontSize: '12px', marginTop: '5px', lineHeight: '1.4', textAlign: 'center' }}>
                  <b>Corporate Office :</b> Chaturavton ka bera, Ponjala, Mata ka than road, Jodhpur,<br />
                ...
                <div style={{ lineHeight: '1.8', fontSize: `${layout.topicsCoveredFontSize}px` }}>
                  {meta.topicsCovered ? (
                    meta.topicsCovered.split('\n').filter(Boolean).map((line, i) => {
                      ...
                      if (parts.length > 1) {
                        return <div key={i}><b>{parts[0]}:</b> {parts.slice(1).join(':')}</div>;
                      }
                      return <div key={i}>{line}</div>;
                    })
                  ) : (
                    <>
                      <div><b>Physics:</b> Motion in 1-D</div>
                      <div><b>Chemistry:</b> Redox Reaction</div>
                      <div><b>Biology:</b> Cell</div>
                    </>
                  )}
                </div>
              </div>
